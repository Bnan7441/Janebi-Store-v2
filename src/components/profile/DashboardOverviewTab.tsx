    ...
    <div className="space-y-6 text-right">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div
          ...
          className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-5 shadow-xs cursor-pointer hover:border-orange-500/30 transition-all group"
        >
        ...
        {orders.length > 0 ? (
          <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 space-y-3">
            <div className="flex items-center justify-between text-xs font-bold">
              <span className="text-gray-900 dark:text-gray-100">کد سفارش: {orders[0].id}</span>
            ...
            <div className="flex items-center gap-3 pt-2">
              {orders[0].items.slice(0, 3).map((item, idx) => (
                <img
                  ...
                  src={item.image}
                  alt={item.title}
                  className="w-12 h-12 rounded-xl object-contain bg-white dark:bg-gray-900 p-1 border border-gray-200 dark:border-gray-700"
                />
              ))}
